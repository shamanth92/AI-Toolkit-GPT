...
export default function Chatbot() {
  ...
  return (
    <div className="flex justify-center items-center gap-6 flex-col w-screen h-screen">
        <div className="w-100 flex flex-col">
            ...
            <div className="w-full h-100 border-1 border-green-500 bg-grey-500 p-2 overflow-y-auto">
                {userPrompt?.map((text, index) => (
                    ...
                    {aiText[index] && <div className="flex justify-end w-full">
      ...
      <div className="h-8 w-8 animate-spin rounded-full border-4 border-gray-300 border-t-green-500" />
            ...
            <div className="w-full">
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="flex">
                        <input type="text"
                            {...register("prompt", { required: "Prompt is required" })} className="border-1 border-green-500 w-full h-20"></input>
                        {errors.prompt && (
                            <p className="text-red-500 text-sm">
                            {errors.prompt.message}
                            </p>
                        )}
                        <button type="submit" className="h-20 text-white bg-green-500 cursor-pointer">Generate text</button>
                    </div>
